...
END {
	...
	for (item in array) {
		if (array[item]["special"] == "typedef") {
			defed_type = array[item]["type"]
			if (!("name" in array[defed_type])) {
				array[defed_type]["name"] = array[item]["name"]
			}
		}
	}
	for (item in array) {
	...
	for (item in array) {
		if (array[item]["name"] == VAR_NAME) {
			...
			print "#define MPERS_" ARCH_FLAG "_" array[type]["name"] " " ARCH_FLAG "_" array[type]["name"]
		}
	}
}
